...
Eduflex is a full-featured Online Learning Management System (LMS) built on the CodeIgniter PHP framework. It provides a platform for educators to create and sell online courses, while offering students an interactive learning environment.
...
- **Wishlist & Cart:** Save courses and purchase multiple courses with multi-currency support.
- **Student Dashboard:** Track enrolled courses, learning progress, and profile settings.
...
### Admin Control Panel
- **User Management:** Manage roles and permissions for Students, Instructors, and Administrators.
- **Category Taxonomy:** Unlimited nested course categories and subcategories.
- **Financial Settings:** Configure global currencies, commission rates, and payment gateways.
...
- XAMPP, WAMP, or equivalent local web server
...
3. **Set Up Database**
   ...
   - Import the database schema from `db/dblms.sql`.

4. **Configure Database Connection**
   Open `application/config/database.php` and update your database credentials if needed:
   ```php
   'hostname' => 'localhost',
   'username' => 'root',
   'password' => '',
...
> Note: Update default passwords after deploying to a production environment.
